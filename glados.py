...
if __name__ == '__main__':
    ...
    for cookie in cookies:
        checkin = requests.post(checkin_url,
                                headers={'cookie': cookie, 'referrer': referrer, 'origin': origin,
                                         'user-agent': useragent, 'content-type': 'application/json; charset=utf-8'},
                                data=json.dumps(payload))
        status = requests.get(status_url, headers={'cookie': cookie, 'referrer': referrer, 'origin': origin,
                                                   'useragent': useragent})
        ...
        if 'message' in checkin.text:
            ...
            if 'Checkin Repeats' in msg:
                sendmsg += '签到失败，重复签到！'
                # print(sendmsg)
            elif 'Checkin! Got' in msg:
    ...
    if pkey != '':
        sendUrl = 'http://pushplus.hxtrip.com/send?token=' + pkey + '&title=GLaDOS签到情况&content=' + sendmsg
        # print(sendUrl)
